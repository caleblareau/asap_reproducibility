...
import_kite_counts <- function(library){
  mtx <- fread(paste0("../data/asapseq/adt/ASAP_",library,"_ADT/featurecounts.mtx.gz"), header = FALSE)
  dim <- mtx[1,]
  mtx <- mtx[-1,]
  matx <- sparseMatrix(i = mtx[[1]], j = mtx[[2]], x = mtx[[3]])
  rownames(matx) <- fread(paste0("../data/asapseq/adt/ASAP_",library,"_ADT/featurecounts.barcodes.txt.gz"), header = FALSE)[[1]]
  colnames(matx) <- paste0(fread(paste0("../data/asapseq/adt/ASAP_",library,"_ADT/featurecounts.genes.txt.gz"), header = FALSE)[[1]])
  return(t(matx))
}
