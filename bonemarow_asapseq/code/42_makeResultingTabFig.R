...
kappa2(mdf[, c("archr_cluster", "predictions_l2")], weight = "unweighted")
...
oo <- dd %>% arrange(desc(prop)) %>% pull(predictions_l2) %>% unique()
dd$order_l2 <- factor(as.character(dd$predictions_l2), levels = oo)

p1 <- ggplot(dd, aes(y = archr_cluster, x = order_l2, fill =  prop)) +
  geom_tile() + pretty_plot(fontsize = 6) + L_border() +
  scale_fill_gradientn(colours = jdb_palette("brewer_heat")) +
  theme(legend.position = "bottom") +
  scale_y_discrete(limits = rev(unique(dd$archr_cluster)))+ 
  labs(y = "ArchR Cluster (accessible chromatin only)", x = "Seurat Projection (Protein tags only)", fill= "% of Seurat projection") +
  theme(axis.text.x = element_text(angle = 45,  hjust = 1))

cowplot::ggsave2(p1, file = "../plots/heatmap_proportion_supplement.pdf",
                 width = 4, height = 2.7)
