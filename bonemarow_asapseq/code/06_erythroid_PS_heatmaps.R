...
m_ps <- mdf %>% dplyr::filter(!is.na(erythroid_PS)) %>% pull(barcode)
...
ordered_cells <- mdf[!is.na(mdf$erythroid_PS),] %>% arrange((erythroid_PS))
...
groups_gs <- sapply(1:n_bins, function(idx){
  ...
  bcs <- ordered_cells %>% dplyr::filter(erythroid_PS >= (idx-1)*multiplier & erythroid_PS < (idx*multiplier)) %>% pull(barcode)
  print(length(bcs))
...
if(FALSE){
  ...
  # Now get gene orders
  vec <- time_df$Gene_symbol; names(vec) <- time_df$Marker_name
  genes_order <- unname(vec[markers_order])
  ...
  cowplot::ggsave2(pS, file = "../plots/adt_chromatin_timing_ery.pdf", width = 1.8, height = 1.8)  
}
