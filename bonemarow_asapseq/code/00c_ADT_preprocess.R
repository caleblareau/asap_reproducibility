...
# Filter for HTO / ArchR valid
colnames(bm) <- paste0(colnames(bm), "-1")
step2bcs <- fread("../data/barcodes/step2_singlets_from_HTO.tsv")[[1]]
bm <- bm[,colnames(bm) %in% step2bcs]

# Export the final list
df <- data.frame(cells = colnames(bm))
write.table(df, file = "../data/barcodes/step3_ADThq.tsv",
            sep = "\t", quote = FALSE, col.names = FALSE, row.names = FALSE)

saveRDS(bm, file = "../output/adt_mat/ASAP_bonemarrow_matrix.rds")
