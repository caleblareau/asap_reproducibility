...
top_k <- pbmc  %>% group_by(channel) %>% top_n(5000, LibraryComplexity)
top_k %>% group_by(channel) %>% summarize(median(LibraryComplexity))
p1 <- ggplot(top_k, aes(x = channel, y = log10(LibraryComplexity), color = channel)) +
